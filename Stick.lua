...
if getgenv().AutomaticTrashFarm then
    return
end
getgenv().AutomaticTrashFarm = true
...
local TrashTriggerFolder = Workspace:WaitForChild("Map"):WaitForChild("ScriptObjects"):WaitForChild("TrashTriggers"):WaitForChild("Stick")
...
local function getStickTool(container)
    for _, tool in ipairs(container:GetChildren()) do
        if tool:IsA("Tool") and string.find(tool.Name, "Stick_") then
            return tool
        end
    end
    return nil
...
task.spawn(function()
    while getgenv().AutomaticTrashFarm do
        task.wait(0.05)
        if IsSelling then
            continue
        end
        ...
        local EquippedStick = getStickTool(Character)
        if not EquippedStick then
            local BackpackStick = getStickTool(LocalPlayer.Backpack)
            if BackpackStick then
                BackpackStick.Parent = Character
                task.wait(0.05)
            else
                continue
            end
